    ...
    if (message.content === discord.decor.Emojis.POTATO) {
      ...
      await message.reply(
        new discord.Embed({
          ...
          }, and now holds **${newCount}** potato${newCount === 1 ? '' : 's'}.`,
      ...
      await message.reply(
        new discord.Embed({
          ...
          description: userMap
            .map(
              (entry) =>
                `\`**${message.author?.username}**#${message.author?.discriminator}\`: ${entry.potatos} potatos`
            )
            .join('\n')
        })
      );
    ...
    potatoSubcommands.raw(
      ...
      async (message) => {
        if (await potatoKV.get<boolean>(`weekly-${message.author.id}`))
          return await message.reply(
            'you already claimed your **weekly** potato!'
          );
        ...
        await message.reply(
          `you claimed your weekly potato, and now holds **${newCount}** potatos.`
        );
      }
    );
